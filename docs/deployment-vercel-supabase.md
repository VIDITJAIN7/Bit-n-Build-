...
1. Vercel is configured by `vercel.json`: repository root, `npm run build`, and `apps/web/dist`. Vite deep links use the SPA rewrite, which excludes `/api`; `api/index.py` exposes the FastAPI app for all nested `/api/*` paths. FastAPI's Python runtime is still Beta, so validate a preview deployment before treating this as a production service.
2. The Supabase migrations recorded in `supabase/migrations/README.md` provision workspace membership, roles, a locked Postgres state record, media metadata, RLS policies, an Auth bootstrap trigger, and a private evidence bucket. The first invited Auth account becomes the initial workspace admin; later invited accounts become workers.
...
6. The local five-second loop is not started in hosted mode. Configure Supabase Cron with `pg_cron` and `pg_net` to call the authenticated API scheduler every minute; store the matching `CRON_SECRET` in Vercel and Supabase Vault. The commander then processes saved admin tasks, with deterministic policy checks at proposal and execution time. The workspace agent starts active, but an empty workspace has no tasks or operating funds. Purchases still cannot move real funds: the production wallet adapter is unconfigured. Telemetry, inventory, workforce, and wallet fields in Connections store only admin-entered connection details; they do not activate a vendor adapter by themselves.
...
| `WORKKITE_AGENT=ai-commander` | Vercel API runtime | The model chooses immediate handling, scheduling, priority and permitted assignee |
| `WORKKITE_RISK_REVIEWER=ai` | Vercel API runtime | Optional separate AI risk assessment |
| `WORKKITE_LLM_BASE_URL` | Vercel API runtime | Selected model provider endpoint |
| `WORKKITE_LLM_MODEL` | Vercel API runtime | Selected provider model name |
| `WORKKITE_LLM_API_KEY` | Vercel API secret | Server-only model credential |
| `CRON_SECRET` | Vercel API secret and Supabase Vault | Authenticates the scheduled agent invocation |
